...
orderEstimatorKnown = @(N,y,a,b) (min(y) - ((N^(-1/a))*b*gamma(1 + (1/a))));
...
for NobsIter=1:length(NobsVec) % loop for samples of varying sizes
    ...
    for NmcIter = 1:Nmc % Nmc monte carlo runs for each sample size
        ...
        % all estimators
        xhatBlue_ = blueEstimator(Nobs,y,beta,alpha);
        xhatOrderKnown_ = orderEstimatorKnown(Nobs,y,beta,alpha);
       ...
       theoreticalVarOrderKnown_(NobsIter,NmcIter) = alpha^2 * Nobs^(-2/beta) * (gamma(1 + (2/beta)) - gamma(1 + (1/beta))^2);
...
uboundOrderKnown = OrderKnownVarsd(NobsIter,1:ceil(length(OrderKnownVarsd(NobsIter,:))*0.02)); % Desired Output
